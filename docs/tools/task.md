...
The wire schema is shape-swapped by `task.batch` (default on). One unit of work is the task item `{ name?, agent?, model?, task, effort?, outputSchema?, schemaMode?, isolated? }`. `isolated` exists only when `task.isolation.mode` is not `none` **and plan mode is disabled**; `effort` exists only when `task.enableEffort=true` (default off).

- **Batch shape** (`task.batch` on): `{ context, tasks: item[] }` — one subagent per item, all run under the same fan-out rules; there is no top-level agent field. `context` is **required** shared background rendered into every spawned subagent's system prompt (`CONTEXT` section); `agent`, `model`, `outputSchema`, and `schemaMode` are per item. `effort` is added only when its setting enables it; `isolated` additionally requires plan mode to be disabled.
...
| `model` | `string` | No | Per-spawn model (`provider/id` or `@role`). Always on the wire (not gated like `effort`). Item field in batch shape, top-level in flat shape. Omit to inherit: per-call `model` → `task.agentModelOverrides[agentName]` → agent frontmatter → session/`@default`. An explicit value is fail-closed — empty, unresolvable, unauthenticatable, session-inherited (`default` / `@default` / `@task` / legacy aliases), or literal `default` (no `@`, case-insensitive) fails the spawn (`StructuredSubagentError` preflight). Parent auth-fallback applies only when the field is absent. |
...
7. Model priority: per-call `model` → `task.agentModelOverrides[agentName]` → agent frontmatter → session/`@default`. An explicit per-call `model` is fail-closed: if it does not resolve or authenticate, the spawn fails (`StructuredSubagentError` preflight). Literal `default` (no `@`, case-insensitive) is rejected; omit the field to inherit. Parent auth-fallback applies only when `model` is absent. Output schema priority: per-call `outputSchema` → agent frontmatter `output` → inherited parent session schema.
...
- Parameter validation failures are returned as normal tool text with empty `results`:
  ...
  - explicit per-call `model` that is empty, session-inherited, unresolvable/unauthenticatable, or literal `default` (fail-closed `StructuredSubagentError` preflight)
